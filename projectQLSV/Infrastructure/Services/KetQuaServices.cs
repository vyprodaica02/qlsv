...
{
    public class KetQuaServices<T> : IRepository<T> where T : KetQua
    {
        private DbSet<T> _services;
        private readonly AppDbContext dbContext;
        public KetQuaServices(AppDbContext dbContext)
        {
        ...
        {
            var lopHocHT = dbContext.ketQuas.FirstOrDefault(x => x.Id == id);

            if (AutoCommitEnabledInternal)
            {
                dbContext.Remove(lopHocHT);
                await CommitAsync();
            }
        }
